from networkx.algorithms.similarity import simrank_similarity_numpy
import torch
...
#set masks via __set_masks__ function
class PGExplainer(BaseExplainer):
    ...
    def __init__(self, model, num_hops: Optional[int] = None, return_type: str = 'log_prob',
                 ...
                 log: bool = True, epochs=30, lr=0.003, temp=(5.0, 2.0), sample_bias=0, align_emb=False, aligner=None, loss='Tgt',directional=False,**kwargs):
        ...
        if self.type == "graph":
            self.expl_embedding = self.model.nhid * 2*kwargs['nlayer'] # src/tgt node embedding
        else:
            ...
            for data in dataloader:        
                ...
                if self.type == 'node':
                    ...
                    for node_idx in data.expl_mask.nonzero().squeeze():
                        ...
                        if self.align_emb:
                            embeds_src, grads_src = self.aligner.get_emb(self.model, x, kwargs['y'], edge_index, mapping)
                    ...
                    if True:
                        ...
                        init = {}
                        init['edge_mask'] = mask
                        self.__set_masks__(data.x, edge_index, init=init)
                        out = self.model(data.x, edge_index, batch=data.batch)
                        if self.return_type == 'regression':
                        ...
                        if self.align_emb:
                            embeds_src, grads_src = self.aligner.get_emb(self.model, data.x, data.y, edge_index,-1, batch=data.batch)
