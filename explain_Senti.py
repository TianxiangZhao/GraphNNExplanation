...
if args.log:
    tb_path = './tensorboard_journal/{}/{}_{}/model{}/edgesize{}ent{}epoch{}exploss{}directional{}align{}aligner{}grad{}weight{}'.format(args.dataset,args.task, args.explainer, args.model,args.edge_size,args.edge_ent,args.epochs, args.expl_loss,args.directional, args.align_emb,args.aligner, args.align_with_grad, args.align_weight)
...
if args.datatype == 'node':
    ...
    for ind, data in enumerate(dataset):
        ...
        sentence_list.append(dataset.supplement['sentence_tokens']['{}'.format(ind)])
...
if args.datatype == 'node':
    ...
    for graph_id, data in enumerate(graph_list):
        ...
        tokens = sentence_list[graph_id]

        #obtain ground-truth explanation for path-structured explanation on target node
        node_feat_mask, edge_mask, edge_index, center_node, kwargs = explainer.explain_graph(data.x, data.edge_index, y=data.y, batch=batch)
        ...
        #draw expl graph
        show_edge_num = 8
        show_edge_num = int(show_edge_num*1.5) if show_edge_num*1.5 < edge_index.shape[1] else edge_index.shape[1]
        try:
            threshold = edge_mask.topk(k=show_edge_num)[0][-1].item()
        except:
            ipdb.set_trace()
        fig, G = explainer.visualize_subgraph(-1, edge_index.cpu(), edge_mask.cpu(), y=kwargs['pred_y'].cpu(),threshold=threshold, node_depict=tokens)
        ...
        if args.log:
            ...
            writer.add_text('sentence', ' '.join(tokens),graph_id)
...
fig_pos = plot_chart([Pos_fidel_list],[args.explainer], x_start=1,y_name='Agreement Rate')
fig_neg = plot_chart([Neg_fidel_list],[args.explainer], x_start=1, y_name='Agreement Rate')
if args.log:
