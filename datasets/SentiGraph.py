...
def split(data, batch):
    ...
    slices['expl_mask'] = torch.arange(0, batch[-1] + 2, dtype=torch.long)
    return data, slices
...
def read_sentigraph_data(folder: str, prefix: str):
    ...
    data = Data(x=x, edge_index=edge_index, y=y, expl_mask=(y==y))
...
if __name__ == '__main__':
    ipdb.set_trace()
    dataset = SentiGraphDataset(root='./datasets/SentiGraph', name='Graph-Twitter')
